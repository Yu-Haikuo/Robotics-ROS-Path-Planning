...
std::vector<std::array<int, 2>> findPath(int idx_x, int idx_y, int goal_x, int goal_y)
{
  ...
  while (!queue.empty())
  {
    ...
    Node *cur_node = queue.front(); // Access the first element in the queue. 
    queue.pop_front();              // Delete the first element from the queue. 
    int cur_x = cur_node->x;        
    int cur_y = cur_node->y;
    int cur_cost = cur_node->cost;

    // ---------------------- (2) Find the path if goal is found, and break loop -------------------------
    if (cur_x == goal_x && cur_y == goal_y)   // We have reached our destination. 
    {
      Node *node = cur_node;
      do
      {
        path.push_back({node->x, node->y}); // Add the node at the back of the queue. 
        node = node->parent;                // While the node has a parent, we go to its parent. 
      } while (node != nullptr);            // If the node has no parent, then end the do-while loop. This loop will add all nodes in this available path into the queue.
      break;
    }
    ...
    /* 
     * I made some modification here. I changed the order of checking neighbors. 
     * Now the program will check west neighbor first before north neighbor. 
     * This is to prevent the robot car from going to the wrong north direction first and then having to make a U turn because of a obstacle. 
     * After modification, it will go directly to the west direction. Details are in the report and video! 
     */

    // === (b) check WEST neighbor ===
    if (!cur_cell->walls[1] && cur_y < MAP_MAX_Y - 1)
    {                                 p            // west wall does not exist AND west cell exists ==> west cell is accessible
